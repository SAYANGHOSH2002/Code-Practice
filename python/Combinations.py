...
def generate_combinations(length):
    def backtrack(combination):
        if len(combination) == length:
            ...
            return
        for char in range(ord(combination[-1]) + 1, 123):
            backtrack(combination + chr(char))
...
# Take input from the user for the length of combinations
rank = int(input())
length = int(input())
